...
public class Assembler {
    ...
    public Assembler() {
        ...
        try {
            ...
            for (int i = 0; i < machine_code.size(); i++) {
                String Decimal = String.valueOf(decimalValueOf(machine_code.get(i)));
    ...
    private void MachineCode() {
        for (int i = 0; i < instruction.size(); i++) {
            ...
            if (instruction.get(i).matches(Format.R_format)) {
                ...
                binary.append(fillTwoComplement(number));
            }
    ...
    public static String fillTwoComplement(String number){
        int n = Integer.parseInt(number);
        String twoComplement;
        StringBuilder str;
        String result;
        if(n < 0){
            ...
            str = new StringBuilder(twoComplement);
            StringBuilder extend = new StringBuilder();
            while (str.length() + extend.length() < 32) {
                extend.append("0");
            }
            result = extend.append(str).toString();
        }
    ...
    private void printDecimalMachineCode() {
        for (int i = 0; i < machine_code.size(); i++) {
            System.out.println(decimalValueOf(machine_code.get(i)));
        }
    }

    public static int decimalValueOf(String number){
        double result = 0;
        if(number.charAt(0) == '1'){
            result = result -Math.pow(2,31) + Long.parseUnsignedLong(number.substring(1,number.length()),2);
        }
        else{
